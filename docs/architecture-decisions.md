...
**Upgrade path:** Introduce a `FileStoragePort` (out port) with an adapter for S3/MinIO/Azure Blob. The domain entity would hold only a `ProfilePictureId` value object instead of bytes. The web adapter resolves the ID to a URL when building responses. Swapping providers means writing a new adapter, nothing else changes.